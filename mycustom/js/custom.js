...
$.ajax({
  ...
  .done(function (data) {
    ...
    if (data != 0) {
      ...
      $('select[name="e_assigned"]').html(
        "<option disabled selected>Please select assign room</option>"
      );
    }
  })
  .fail(function (data) {
...
$.ajax({
  ...
  $('select[name="borrow_membername"]')
    .html("<option disabled selected></option>" + b)
    .select2({
...
$("body").on(
  "click",
  ".equipment-forminfo .cancel-equipmentinfo",
  function (e) {
    e.preventDefault();
...
$.ajax({
  ...
  $('select[name="reserve_room"]').html(
    "<option disabled selected>Please select room</option>" + b
  );
